...
TrueNAS automatically generates a root dataset for new pools.
This root dataset inherits the pool encryption state from the **Encryption** option on the **[Pool Manager]({{< relref "PoolManagerScreens.md" >}})** screen used to create the pool.
Enable the **Encryption** pool option to create the root dataset with encryption and encrypt all datasets and Zvols created in that pool.

{{< expand "I'm seeing an unencrypted dataset in an encrypted pool?" "v" >}}
In versions prior to SCALE 22.12.3, it was possible to erroneously create an unencrypted dataset in an encrypted pool.
This behavior was corrected in SCALE 22.12.3 and later but datasets created in this way were preserved to prevent any disruptive changes on update.
If you find an unencrypted dataset in an encrypted pool on your system, review your security requirements and, if necessary, migrate data from the unencrypted dataset to a new encrypted one.
{{< /expand >}}

Leaving the **Encryption** option on the **Pool Manager** screen disabled creates the pool with an unencrypted root dataset.
It is possible to create both unencrypted and encrypted datasets within a pool with an unencrypted root dataset.
New encrypted datasets within an unencrypted pool encrypts any new datasets or Zvols created within that encrypted dataset.
When the system has a single pool, do not enable the **Encryption** option for the pool.

{{< expand "Can I change dataset encryption?" "v" >}}
You can change the type of encryption of an encrypted dataset (root or child dataset) from key to passphrase, but encrypted datasets cannot become unencrypted after creation.
