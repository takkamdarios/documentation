...
Remote replication can occur between your TrueNAS SCALE system and another TrueNAS system where you want to use to store your replicated snapshots. 

With the implementation of rootless login and the admin user, setting up replication tasks as an admin user differs from setting up replication tasks when logged in as root. 
...
It also covers the related steps you should take before to configuring a replication task. 
...
The wizard helps create a new SSH connection and automatically creates a periodic snapshot task for sources with no existing snapshots.
...
3. Go to **Data Protection** and click **Add** on the **Replication Tasks** widget to open the **Replication Task Wizard**. Configure the following settings:
   ...
   c. Browse to the source pool/dataset(s), then click on the dataset(s) to populate the **Source** with the path. 
      You can select multiple sources or manually type the names into the **Source** field separated by commas. 
   ...
   f. Select the **Include snapshots with the name** radio button you want to use. If **Naming Schema**, you must enter a schema in **Naming Schema**.
      ...
      Enter the name and [strftime(3)](https://man7.org/linux/man-pages/man3/strftime.3.html) %Y, %m, %d, %H, and %M strings that match the snapshots to include in the replication. Separate entries by pressing <kbd>Enter</kbd>. The number of snapshots matching the patterns displays.

   g. (Optional) Enter a name for the snapshot in **Task Name**. 
      SCALE populates this field with the default name using the source and destination paths separated by a hyphen, which can make locating the snapshot in destination dataset challenging. 
      To make it easier to find the snapshot, give it an easily identifiable name. For example, a replicated task named *dailyfull* for a full file system snapshot taken daily. 
